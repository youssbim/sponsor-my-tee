-- Outbids must beat the leader by at least 20% (and at least p_increment euros), rounded up to a multiple of 5.
drop function if exists public.confirm_bid(uuid, text, integer);
create or replace function public.confirm_bid(
  p_bid uuid,
  p_payment_intent text,
  p_increment integer,
  p_rate numeric default 0.2,
...
revoke execute on function public.confirm_bid(uuid, text, integer, numeric, integer) from public, anon, authenticated;
grant execute on function public.confirm_bid(uuid, text, integer, numeric, integer) to service_role;
